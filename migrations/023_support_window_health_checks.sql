...
BEGIN
  ...
  IF v_parse_failed_24h > 0 THEN
    INSERT INTO public.system_alerts (severity, category, message, metadata, raised_at)
    ...
    WHERE NOT EXISTS (
      ...
      WHERE resolved_at IS NULL AND metadata->>'check'='parse_failures_24h'
  ...
  IF v_ingest_queue > 20 THEN
    ...
    WHERE NOT EXISTS (
      SELECT 1 FROM public.system_alerts
      WHERE resolved_at IS NULL AND metadata->>'check'='ingest_queue_depth'
        AND raised_at >= v_now - interval '12 hours'
    );
  ...
  IF v_auto_failed_24h > 3 THEN
    ...
    WHERE NOT EXISTS (
      SELECT 1 FROM public.system_alerts
      WHERE resolved_at IS NULL AND metadata->>'check'='automation_failed_24h'
        AND raised_at >= v_now - interval '12 hours'
    );
    v_raised := v_raised + (CASE WHEN FOUND THEN 1 ELSE 0 END);
  END IF;
  ...
  IF v_stale_alerts > 0 THEN
    ...
    WHERE NOT EXISTS (
      SELECT 1 FROM public.system_alerts
      WHERE resolved_at IS NULL AND metadata->>'check'='stale_alerts_14d'
  ...
  IF v_overdue_taxes > 0 THEN
    ...
    WHERE NOT EXISTS (
      SELECT 1 FROM public.system_alerts
      WHERE resolved_at IS NULL AND metadata->>'check'='overdue_taxes_3d'
