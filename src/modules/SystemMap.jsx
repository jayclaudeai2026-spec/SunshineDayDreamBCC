...
function renderInline(text) {
  ...
  let idx = 0;
  const patterns = [
    { re: /`([^`]+)`/,           render: (m, k) => <code key={k} className="px-1 py-0.5 rounded bg-ia-page text-xs font-mono">{m[1]}</code> },
    { re: /\*\*([^*]+)\*\*/,     render: (m, k) => <strong key={k}>{m[1]}</strong> },
    { re: /\*([^*]+)\*/,         render: (m, k) => <em key={k}>{m[1]}</em> },
    { re: /\[([^\]]+)\]\(([^)]+)\)/, render: (m, k) => <a key={k} href={m[2]} target="_blank" rel="noreferrer" className="text-ia-teal hover:underline">{m[1]}</a> },
  ];
  while (rest.length > 0) {
    let earliest = null;
...
function renderMarkdown(md) {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('```')) {
      ...
      i++; // skip closing
      blocks.push(
        <pre key={blocks.length} className="bg-ia-page border border-ia-border rounded-md px-3 py-2 my-3 overflow-x-auto text-xs">
          <code className="font-mono">{buf.join('\n')}</code>
          {lang && <div className="text-[10px] uppercase tracking-wide text-ia-muted mt-1">{lang}</div>}
        </pre>,
      );
      continue;
    }
    ...
    if (hMatch) {
      ...
      const text = hMatch[2];
      const cls = ['text-base font-semibold mt-4 mb-2 text-ia-navy', 'text-sm font-semibold mt-3 mb-1 text-ia-navy', 'text-xs font-semibold mt-2 mb-1 text-ia-navy uppercase tracking-wide', 'text-xs font-semibold mt-2 mb-1 text-ia-muted'][Math.min(level - 1, 3)];
        ...
        <div key={blocks.length} className="my-3 overflow-x-auto">
          <table className="text-xs border-collapse w-full">
            <thead>
              <tr>
                {header.map((h, hi) => (
                  <th key={hi} className="text-left font-semibold border-b border-ia-border px-2 py-1 text-ia-navy">{renderInline(h)}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {rows.map((r, ri) => (
                <tr key={ri} className="hover:bg-ia-page">
                  {r.map((c, ci) => (
                    <td key={ci} className="border-b border-ia-border px-2 py-1 align-top">{renderInline(c)}</td>
                  ))}
                </tr>
              ))}
            </tbody>
    ...
    if (line.startsWith('> ')) {
      ...
      blocks.push(
        <blockquote key={blocks.length} className="border-l-2 border-ia-teal pl-3 my-2 text-sm text-ia-muted italic">
    ...
    blocks.push(
      <p key={blocks.length} className="my-2 text-sm leading-relaxed text-ia-ink">
...
function CategoryBadge({ category }) {
  return (
    <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium uppercase tracking-wide bg-ia-page text-ia-muted border border-ia-border">
...
function StaleIndicator({ verifiedAt }) {
  ...
  if (days == null) {
    return (
      <span className="inline-flex items-center gap-1 text-[11px] text-ia-warning">
  ...
  return (
    <span className="inline-flex items-center gap-1 text-[11px] text-ia-muted">
...
function PageCard({ page, onOpen }) {
  ...
  return (
    <button
      ...
      className="w-full text-left ia-card hover:border-ia-teal transition-colors cursor-pointer"
    >
      <div className="flex items-start justify-between gap-2 mb-1">
        <h4 className="text-sm font-semibold text-ia-navy">{page.title}</h4>
        <CategoryBadge category={page.category} />
      </div>
      <div className="text-xs text-ia-muted line-clamp-2 mb-2">{preview}{preview.length === 200 ? '…' : ''}</div>
      <div className="flex items-center justify-between">
        <code className="text-[10px] text-ia-muted font-mono">{page.slug}</code>
  ...
  return (
    <div>
      ...
      <div className="ia-card">
        <div className="flex items-start justify-between gap-2 mb-1">
          <h2 className="text-ia-navy">{page.title}</h2>
          <CategoryBadge category={page.category} />
        </div>
        <div className="flex items-center gap-3 text-[11px] text-ia-muted mb-3">
        ...
        {related.length > 0 && (
          <div className="mt-6 pt-3 border-t border-ia-border">
            <div className="text-[11px] text-ia-muted uppercase tracking-wide mb-2 inline-flex items-center gap-1">
  ...
  return (
    <div>
      ...
      <div className="ia-card space-y-3">
        <h3 className="text-ia-navy">{isNew ? 'New system map page' : 'Edit page'}</h3>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-xs font-medium text-ia-navy mb-1">Slug</label>
            <input
              value={slug}
              onChange={(e) => setSlug(e.target.value)}
              disabled={!isNew}
              placeholder="lowercase-hyphenated-id"
              className="w-full px-2 py-1.5 border border-ia-border rounded-md text-sm font-mono disabled:opacity-60 bg-ia-card"
            />
          </div>
          <div>
            <label className="block text-xs font-medium text-ia-navy mb-1">Sort order</label>
            <input
              type="number"
              value={sortOrder}
              onChange={(e) => setSortOrder(e.target.value)}
              className="w-full px-2 py-1.5 border border-ia-border rounded-md text-sm bg-ia-card"
            />
          </div>
        </div>

        <div>
          <label className="block text-xs font-medium text-ia-navy mb-1">Title</label>
          <input
            value={title}
        ...
        <div>
          <label className="block text-xs font-medium text-ia-navy mb-1">Category</label>
          <select
            value={category}
        ...
        <div>
          <label className="block text-xs font-medium text-ia-navy mb-1">Related slugs (comma-separated)</label>
          <input
            value={relatedSlugs}
            onChange={(e) => setRelatedSlugs(e.target.value)}
            placeholder="slug-one, slug-two"
            className="w-full px-2 py-1.5 border border-ia-border rounded-md text-sm font-mono bg-ia-card"
          />
        </div>

        <div>
          <label className="block text-xs font-medium text-ia-navy mb-1">
          ...
          <textarea
            ...
            rows={24}
            className="w-full px-2 py-1.5 border border-ia-border rounded-md text-xs font-mono bg-ia-card"
          />
        </div>
      </div>
    </div>
...
export default function SystemMap() {
  ...
  if (error) {
    return (
      <div className="ia-card text-sm text-red-700">
        Failed to load system_map: {error.message}
        <button onClick={() => refetch()} className="ia-button-ghost text-xs ml-3"><RefreshCw size={12} /> Retry</button>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="relative mb-4">
        <Search size={14} className="absolute left-2.5 top-2.5 text-ia-muted pointer-events-none" />
        <input
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          placeholder="Search titles, slugs, content…"
          className="w-full pl-8 pr-3 py-2 border border-ia-border rounded-md text-sm bg-ia-card"
        />
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        Array.from(grouped.entries()).map(([cat, list]) => (
          <div key={cat} className="mb-6">
            <div className="text-[11px] font-semibold text-ia-muted uppercase tracking-wide mb-2">
